...
//activates a custom control (and related keyboard shortcuts with the buttons activated below)
...
var FADE_TIME = 2000;
//If there is only one path, it will show the option. Otherwise, the option will be skipped.
var SHOW_ONE_OPTION = true;
//If true, a choice delay time of 0 will display the choices a set number of seconds before the video ends. The second variable determines how many seconds before the end the choices should appear.
var CHOICE_DELAY_ON_VIDEO_END = true;
var SECONDS_BEFORE_END = 19;
///////////////////////////////////////////////////////////////////
var GAME = {};
var videoObjects = {};
var preloadInterval;
...
function initGame() {
	$.get(FILE, function(data) {
		...
		for (id in GAME) {
			...
			if (videoTag != null) {
				...
				if (PAUSE_ON_CLICK) $(video).on('click touch', playPause);
				else if (VIDEO_CONTROLS) $(video).on('click touch', revealMenu);
				//save the reference
	...
	if (!AUTOCHOICE) $("#bar").css("visibility", "hidden");
}
...
function update() {
	if (currentVideo != null) {
		...
		if (!choicesOn && currentVideo.currentTime * 1000 > choicesTime) {
			...
			if (currentChoices.length == 1 && (instantChoice || (!instantChoice && choicesTime == 0)) && SHOW_ONE_OPTION == false) {
				...
				if (ANIMATED) $("#choicesContainer").css("animation-play-state", "running");
				else {
	...
	if (currentVideo != null) {
		...
		if (nextChoices.length == 1) {
			...
			if (!choicesOn) {
                ...
                if (CHOICE_DELAY_ON_VIDEO_END && getChoicesTime(GAME[currentPassage].text) == 0) {
					var nextChoicesTime = (currentVideo.duration - SECONDS_BEFORE_END) * 1000;
				}
				if (nextChoicesTime != 0) currentVideo.currentTime = nextChoicesTime / 1000;
			} else {
				//choices are on
				currentVideo.currentTime = currentVideo.duration;
			}
		...
		if (currentPassage != null)
			if (currentVideo != null) {
				if (currentVideo.paused) {
					...
					currentVideo.pause();
					if (VIDEO_CONTROLS) {
						if (!choicesOn) $("#menuContainer").css("display", "block");
...
function videoEnded() {
	...
	if ((AUTOCHOICE && currentChoice == null) || currentChoices.length == 1) {
		...
		//no choice
		if (!AUTOCHOICE && currentVideo != null) currentVideo.pause();
	} else {
...
function displayPassage(pId) {
	...
	if (videoObjects[pId] != null) {
		...
		//get the control delay in milliseconds
		if (CHOICE_DELAY_ON_VIDEO_END && getChoicesTime(GAME[pId].text) == 0) {
			choicesTime = (currentVideo.duration - SECONDS_BEFORE_END) * 1000;
		} else {
			choicesTime = getChoicesTime(GAME[pId].text);
		}
		console.log(choicesTime);
	...
	for (var i = 0; i < arr.length; i++) {
		...
		if (header != null) {
			...
			if (roomArr != null) {
				...
				if (room.match(/\W/g) != null) parsingErrors += "Room id error: room id must not contain special characters or spaces - triggered by " + room + "\n";
			}
			//remove leading and trailing whitespaces
			id = id.replace(/^\s+|\s+$/g, "");
			if (id.match(/\W/g) != null) parsingErrors += "Twee file error: passage title must not contain special characters or spaces - triggered by " + id + "\n";
			else {
...
function getChoicesTime(txt) {
	...
	if (macro != null) {
		...
		if (t != null) {
			...
			if (a[a.length - 1] != null) time += parseFloat(a[a.length - 1]);
			//minutes
			if (a[a.length - 2] != null) time += parseFloat(a[a.length - 2]) * 60;
			//HOURS???
			if (a[a.length - 3] != null) time += parseFloat(a[a.length - 3]) * 60 * 60;
			time *= 1000;
		}
	}
	return time;
...
function revealMenu() {
	if (VIDEO_CONTROLS && currentVideo != null) {
		if (!choicesOn) $("#menuContainer").css("display", "block");
		if (mouseMoveInterval != null) clearTimeout(mouseMoveInterval);
